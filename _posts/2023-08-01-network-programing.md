...
| OSI七层协议 | TCP/IP四层协议                    | 五层协议   | 作用                                                         |
| ----------- | --------------------------------- | ---------- | ------------------------------------------------------------ |
| 应用层      | 应用层(各种应用协议：TELNET、FTP) | 应用层     | 应用层的任务是通过进程间交互来完成特定网络的应用（例如：域名系统DNS，HTTP协议） |
| 表示层      | 运输层（TCP/UDP）                 | 传输层     | 传输层的任务是负责向进程间的通信提供数据传输服务（例如：TCP、UDP） |
| 会话层      | 网际层（IP）                      | 网络层     | 网络层的任务是选择合适的网间路由和交换节点，确保封装的IP数据报及时传送（网络层使用：IP协议） |
| 运输层      | 网络接口层                        | 数据链路层 | 数据链路层的主要任务就是将网络层交过来的IP数据报组成帧在链路上进行数据传输。（链路层只支持帧之间传输）。 |
| 网络层      |                                   | 物理层     | 物理层的任务是根据数据单位 Bit 实现相邻计算机节点的透明传送。 |
| 数据链路层  |                                   |            |                                                              |
| 物理层      |                                   |            |                                                              |
...
**三次握手定义**：建立TCP连接，目的是确保可靠的通信通道。

双工、可靠是由重传机制保证。

第一次握手、客户端发送带有SYN标志的数据包、
		第二次握手、服务端接收之后基于SYN标志的数据包回传ACK标志的数据包给客户端。
		第三次握手，客户端再把ACK发给服务端。这时就建立通信通道成功了！

> 四次挥手

**四次挥手定义**：客户端与服务端四次挥手的目的是断开TCP连接
		第一次：客户端发送 FIN（结束），请求关闭与服务端的连接。
		第二次：服务端收到 FIN，回发 ACK（确认），序号加一，一个FIN占用一个序号。
		第三次：然后服务端发送FIN（结束）给客户端，关闭与客户端的连接。（注意：这里是半关闭状态，不是真正的关闭）
		第四次：客户端确认序号+1后，最后发送ACK表示确认，服务端就会真正的关闭连接。
...
BIO（同步阻塞IO模型）：数据的读取和写入必须在一个线程内等待期完成。

问题：高并发情况会导致线程堆栈溢出，从而导致创建失败，最终导致进程宕机或者僵死。

NIO（同步非阻塞IO模型）：本质解决高并发带来的问题，通过面向缓存、基于通道操作IO的方式提升数据读取和写入的性能。

同步与异步、阻塞与非阻塞

NIO又称为 I/O多路复用模型，具有同步非阻塞特点，通过轮询机制实现反复调用read不会阻塞。

AIO 是基于事件和回调机制实现，应用操作会直接返回，不会阻塞，当后台处理完成，会通知操作系统通知相应的线程进行后续操作。
